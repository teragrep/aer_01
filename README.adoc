...
= AER_01 (Teragrep syslog bridge for Microsoft Azure EventHub)
...
* Currently, `NLFPlugin` is the default plugin, with a fallback to `DefaultPlugin` is used. If other type of additional message processing is required,
